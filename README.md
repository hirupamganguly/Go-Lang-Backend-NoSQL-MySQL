...
REST-API:
An API is an application programming interface. It is a set of rules that allow programs to talk to each other. The developer creates the API on the server and allows the client to talk to it. In REST, one of these rules states that you should be able to get a piece of data (called a resource) when you link to a specific URL. Each URL is called a request while the data sent back to you is called a response. 
The endpoint (or route) is the URL you request for.
Let this is the URL you hit from Browser: /api/book/{id}
The Response you get is: The details of the particular book you specified by ID. 

 The root-endpoint ` / ` is the starting point of the API you’re requesting from. The path ` api/book/{id} `determines the resource you’re requesting for. 
...
This header tells the server to expect JSON content.
